...
- **Quiet execution rhythm.** When a sequence of edits is obvious and planned, run them in one
  ...
  overhead and fragments the work.
...
In the comparison, one model worked in long quiet bursts and closed each section with a single
structured briefing; the other narrated step-by-step. The rhythm difference was real (it held
within the same work phase, so it's not purely a confound). The briefing *format* — heading +
sections + status — is fully promptable.
...
- This is mostly an interaction-style preference → at most one always-on line, and only if your
  current default over-narrates.
- If you have a reporting-format standard already → the section-end briefing likely belongs there;
  don't add a second format.
